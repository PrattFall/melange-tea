...
let attempt (resultToMessage : ('succeed, 'fail) result -> 'msg)
    (task : ('succeed, 'fail) t) : 'msg Tea_cmd.t =
  attemptOpt (fun v -> (Some (resultToMessage v) [@explicit_arity])) task

let ignore task = attemptOpt (fun _ -> None) task

let succeed (value : 'v) : ('v, 'e) t =
  (Task (fun cb -> cb (Ok value [@explicit_arity])) [@explicit_arity])

let fail (value : 'v) : ('e, 'v) t =
  (Task (fun cb -> cb (Error value [@explicit_arity])) [@explicit_arity])

let nativeBinding (func : (('succeed, 'fail) result -> unit) -> unit) :
    ('succeed, 'fail) t =
  (Task func [@explicit_arity])
...
let fromResult : ('success, 'failure) result -> ('success, 'failure) t =
  function
  | Ok s -> succeed s
  | Error err -> fail err
...
let testing () =
  let doTest expected ((Task task) [@explicit_arity]) =
    let testAssert v =
      ...
      else Js.log (("FAILED:" [@reason.raw_literal "FAILED:"]), expected, v)
    in
    task testAssert
  in
  let s = succeed 42 in
  ...
  let () = doTest (Ok ("2" [@reason.raw_literal "2"]) [@explicit_arity]) a2 in
  ...
  let () = doTest (Ok ("9" [@reason.raw_literal "9"]) [@explicit_arity]) m3 in
  ...
  let e0 =
    ...
    |> onError (fun _msg -> succeed 42)
  in
  let () = doTest (Ok 42 [@explicit_arity]) e0 in
  let e1 =
    fail ("file not found" [@reason.raw_literal "file not found"])
    |> onError (fun _msg -> fail 42)
  in
  ...
  in
  let () =
    doTest (Error ("42" [@reason.raw_literal "42"]) [@explicit_arity]) n0
  in
  let n1 =
    sequence
      [
  ...
  in
  let () =
    doTest (Error ("3.14" [@reason.raw_literal "3.14"]) [@explicit_arity]) n1
  in
  let n2 =
    sequence
      [
