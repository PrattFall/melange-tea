...
external set_onreadystatechange : t -> readystatechange -> unit
  = "onReadyStateChange"
[@@mel.set]

external get_onreadystatechange : t -> readystatechange = "onReadyStateChange"
[@@mel.get]
...
external set_onloadstart : t -> on_load_start -> unit = "onLoadStart"
[@@mel.set]
...
let getAllResponseHeadersAsList (x : t) :
  ...
  let format_value (s : Js.String.t) =
    ...
    |> Array.map Array.of_seq |> Array.to_list
...
let getAllResponseHeadersAsDict (x : t) :
    (string Map.Make(String).t, errors) result =
...
let set_timeout (t : float) (x : t) : unit = setTimeout x t
let get_timeout (x : t) : float = timeout x
let set_withCredentials (b : bool) (x : t) : unit = set_withCredentials x b
let get_withCredentials (x : t) : bool = withCredentials x
let set_onabort (cb : event_abort -> unit) (x : t) : unit = set_onabort x cb
let get_onabort (x : t) : event_abort -> unit = onabort x
let set_onerror (cb : event_error -> unit) (x : t) : unit = set_onerror x cb
