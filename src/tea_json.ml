module Decoder = struct
  ...
  type ('input, 'result) t = Decoder of ('input -> ('result, error) result)
  (*
  ...
  let list (Decoder decoder) =
    Decoder
      (fun value ->
        ...
        | JSONArray a -> (
            let parse v =
              match decoder v with Ok r -> r | Error e -> raise (ParseFail e)
            in
  ...
  let maybe (Decoder decoder) =
    Decoder
      (fun value ->
        match decoder value with Ok r -> Ok (Some r) | Error _ -> Ok None)
  ...
  let error = function Error e -> Some e | _ -> None
  let first fst = function Error _ as e -> e | Ok _ -> fst
  let error_of_first fst = function Error e -> Some e | Ok _ -> error fst
  ...
  let map2 mapper (Decoder decoder1) (Decoder decoder2) =
    Decoder
      (fun value ->
        ...
        | e1, e2 -> (
            match error_of_first e1 e2 with
  ...
  let succeed v = Decoder (fun _value -> Ok v)
  let fail e = Decoder (fun _value -> Error e)
  let value = Decoder (fun value -> Ok value)
  ...
  let decodeEvent (Decoder decoder) (value : 'e Dom.event_like) =
  ...
  let decodeString decoder string =
    ...
    with (* | JsException e -> Error ("Given an invalid JSON: " ^ e) *)
    | _ ->
