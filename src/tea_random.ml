...
type 'typ t = Generator of (Random.State.t -> 'typ)

let bool = Generator (fun state -> Random.State.bool state)
...
let list count (Generator genCmd) =
  let rec buildList state i acc =
    if i > 0 then buildList state (i - 1) (genCmd state :: acc) else acc
  in
...
let map4 func (Generator genCmd1) (Generator genCmd2) (Generator genCmd3)
    (Generator genCmd4) =
  Generator
    (fun state ->
      let res1 = genCmd1 state in
...
let map5 func (Generator genCmd1) (Generator genCmd2) (Generator genCmd3)
    (Generator genCmd4) (Generator genCmd5) =
  Generator
    (fun state ->
      let res1 = genCmd1 state in
      let res2 = genCmd2 state in
...
type seed = Seed of Random.State.t
